...
(() => {
  ...
  const siteUpdateTitles = [
    {
      id: "site-update-month-tba-display",
      title: { ko: "월 미정 작품 표시", ja: "月未定作品の表示", en: "Month-TBA releases" }
    },
    {
      id: "site-update-previous-series-streaming",
      title: { ko: "이전 시리즈 정주행", ja: "過去シリーズ一気見", en: "Previous-series streaming" }
    },
    {
      id: "site-update-official-links",
      title: { ko: "공식 사이트 링크", ja: "公式サイトリンク", en: "Official site links" }
    },
    {
      id: "site-update-pv-links",
      title: { ko: "공식 PV 링크", ja: "公式PVリンク", en: "Official PV links" }
    },
    {
      id: "site-update-verification",
      title: { ko: "작품 정보 검증", ja: "作品情報の検証", en: "Anime information verification" }
    },
    {
      id: "site-update-ui",
      title: { ko: "사이트 UI 개선", ja: "サイトUI改善", en: "Site UI improvements" }
    }
  ];

  const knownAnimeIds = new Set(window.animeData.map(anime => anime?.id).filter(Boolean));
  siteUpdateTitles.forEach(entry => {
    if (knownAnimeIds.has(entry.id)) return;
  ...
  const correctedTitleAnime = window.animeData.find(anime => anime?.id === "dating-a-dark-summoner");
  if (correctedTitleAnime) {
    correctedTitleAnime.title = {
      ...correctedTitleAnime.title,
      ko: "다크서머너와 썸을 탔다"
    };
  }
  ...
  const additions = [
    {
      ...
      animeId: "site-update-month-tba-display",
    ...
    },
    {
      ...
      animeId: "site-update-previous-series-streaming",
      changedAt: "2026-09-05",
    ...
    },
    {
      ...
      animeId: "site-update-official-links",
      changedAt: "2026-09-05",
      type: "official-link",
      summary: {
    ...
    },
    {
      ...
      animeId: "site-update-verification",
      changedAt: "2026-09-05",
      type: "source",
      summary: {
