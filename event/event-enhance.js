(() => {
  ...
  const copy = {
    ko: {
      ...
      wishlistReady: '참여 조건을 충족했습니다.',
      wishlistNeed: (remaining) => `위시리스트에 기대작을 ${remaining}개 더 추가하면 참여할 수 있습니다.`,
    ...
    ja: {
      ...
      wishlistReady: '参加条件を満たしています。',
      wishlistNeed: (remaining) => `あと${remaining}作品をウィッシュリストに追加すると参加できます。`,
      browseAnime: '作品を探す',
    ...
    en: {
      ...
      wishlistReady: 'You meet the participation requirement.',
      wishlistNeed: (remaining) => `Add ${remaining} more title${remaining === 1 ? '' : 's'} to your wishlist to participate.`,
  ...
  async function loadDefinition() {
    ...
    const { data } = await db.from('event_definitions')
      .select('id,starts_at,ends_at,eligibility_type,eligibility_value')
      .eq('id', eventId)
      .maybeSingle();
