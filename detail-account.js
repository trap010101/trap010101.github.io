(() => {
  ...
  const copy = {
    ko: { login: '로그인', account: '계정', title: '간편 로그인', logout: '로그아웃', close: '닫기', add: '위시리스트에 추가', remove: '위시리스트에서 제거', description: 'Google 계정으로 로그인하면 위시리스트가 기기 간에 동기화됩니다.', error: 'Google 로그인에 실패했습니다.' },
    ja: { login: 'ログイン', account: 'アカウント', title: 'かんたんログイン', logout: 'ログアウト', close: '閉じる', add: 'ウィッシュリストに追加', remove: 'ウィッシュリストから削除', description: 'Googleでログインするとウィッシュリストを端末間で同期できます。', error: 'Googleログインに失敗しました。' },
    en: { login: 'LOGIN', account: 'ACCOUNT', title: 'Quick login', logout: 'Sign out', close: 'Close', add: 'Add to wishlist', remove: 'Remove from wishlist', description: 'Sign in with Google to sync your wishlist across devices.', error: 'Google sign-in failed.' }
  };
  ...
  modal.hidden = true;
  modal.innerHTML = `
    <div class="detail-account-backdrop" data-detail-account-close></div>
    <section class="detail-account-dialog" role="dialog" aria-modal="true" aria-labelledby="detailAccountTitle" tabindex="-1">
      <div class="detail-account-head">
        <div><div class="detail-account-kicker">newani.me</div><h2 id="detailAccountTitle"></h2><p class="detail-account-description"></p></div>
        <button class="detail-account-close" type="button" data-detail-account-close>×</button>
      </div>
      <div class="detail-account-body"></div>
      <p class="detail-account-status" role="status" aria-live="polite"></p>
    </section>`;
  ...
  function renderAccount() {
    ...
    modal.querySelector('#detailAccountTitle').textContent = currentUser ? text('account') : text('title');
    ...
    if (currentUser) {
      ...
      if (avatar) icon.innerHTML = `<img src="${escapeHtml(avatar)}" alt="">`;
  ...
  function renderGoogleButton() {
    ...
    try {
      ...
      window.google.accounts.id.renderButton(slot, { type: 'standard', theme: 'filled_black', size: 'large', shape: 'pill', text: 'continue_with', width: Math.min(340, Math.max(220, modal.querySelector('.detail-account-dialog')?.clientWidth - 44 || 300)) });
    } catch (_) {
  ...
  function openModal() {
    ...
    requestAnimationFrame(() => {
      modal.querySelector('.detail-account-close')?.focus();
      renderGoogleButton();
    });
  }
  function closeModal() {
